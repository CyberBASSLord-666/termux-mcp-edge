...
The baseline staged tools remain limited to `runtime_status`, `platform_info`, `android_status`, `project_service_status`, dry-run-first `create_directory`, dry-run-first `copy_file`, `hash_file`, `list_directory`, `path_metadata`, `read_binary_file`, `read_binary_range`, `read_file`, bounded literal `search_text`, and dry-run-first `write_file`. Directory mutation is separately default-disabled and requires one short-lived, target-bound, single-use `MCP-Capability-Grant`; explicit `dry_run:false` alone is denied. File copy accepts only one no-follow regular source up to 1 MiB, requires an absent safe-rooted destination with an existing parent, publishes mode `0600` without replacement, and returns no content. Whole-file binary read accepts one no-follow regular file up to 1 MiB. Binary range read accepts at most 256 KiB from one no-follow regular file up to 64 MiB and rejects offset-past-EOF or concurrent size changes. Both return only canonical padded RFC 4648 base64 plus fixed size/limit metadata under fixed complete-response ceilings. File hashing streams one no-follow regular file up to 16 MiB through SHA-256 and returns only the digest and byte count. Hashing, metadata, and search remain descriptor-relative and content-private. Separately built and explicitly enabled postures may add bounded `android_battery_status`, `android_volume_status`, `set_android_volume`, or `run_command_profile` under their dedicated contracts. The grant contract is defined in `docs/CREATE_DIRECTORY_CAPABILITY_GRANTS.md`; filesystem contracts are defined in `docs/SAFE_ROOT_DIRECTORY_CREATION.md`, `docs/SAFE_ROOT_FILE_COPY.md`, `docs/SAFE_ROOT_FILE_HASHING.md`, `docs/SAFE_ROOT_BINARY_READS.md`, and `docs/SAFE_ROOT_BINARY_RANGES.md`.
