...
The baseline staged tools remain limited to `runtime_status`, `platform_info`, `android_status`, `project_service_status`, dry-run-first `create_directory`, dry-run-first `copy_file`, `list_directory`, `path_metadata`, `read_file`, bounded literal `search_text`, and dry-run-first `write_file`. File copy accepts only one no-follow regular source up to 1 MiB, requires an absent safe-rooted destination with an existing parent, publishes fixed mode `0600` atomically without replacement, verifies held and published identities, and returns no content. Metadata and search remain descriptor-relative and content-private. Separately built and explicitly enabled postures may add bounded read-only `android_battery_status`, `android_volume_status`, or `run_command_profile`. Provider contracts are defined in `docs/ANDROID_BATTERY_STATUS.md` and `docs/ANDROID_VOLUME_STATUS.md`; filesystem mutation contracts are defined in `docs/SAFE_ROOT_DIRECTORY_CREATION.md` and `docs/SAFE_ROOT_FILE_COPY.md`; the command contract is defined in `docs/command-execution-gate.md`.
