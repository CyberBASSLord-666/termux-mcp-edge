...
The baseline staged tools remain limited to `runtime_status`, `platform_info`, `android_status`, `project_service_status`, dry-run-first `create_directory`, dry-run-first `copy_file`, `find_paths`, `hash_file`, `list_directory`, `path_metadata`, `read_binary_file`, `read_binary_range`, `read_file`, bounded literal `search_text`, and dry-run-first `write_file`. Directory, copy, and file-write mutation are independently default-disabled and each requires one short-lived, operation-bound, single-use `MCP-Capability-Grant`; explicit `dry_run:false` alone is denied. A copy grant binds the authenticated principal/session, both anchored roots and normalized paths, exact single-link source identity/size/high-resolution ctime/SHA-256, absent destination, and no-replace posture. A write grant binds exact UTF-8 content, create-or-replace disposition, and exact old replacement identity. Both use opaque fixed 65-byte payloads that expose only their distinct signed family byte and serialize none of the private binding inputs. Live copy and writes are capped at 1 MiB and publish mode `0600`. Copy and write-create use atomic no-replace; write-replace uses one irreversible exchange and preserves the displaced inode/content in a reserved private quarantine capped at 32 entries and 32 MiB per target parent. Copy stages only inside that hidden mode-`0700` quarantine. Results disclose neither content nor path. Path discovery, binary reads, hashing, metadata, and text search retain their descriptor-relative fixed ceilings and content-private contracts. Separately built and explicitly enabled postures may add bounded `android_battery_status`, `android_volume_status`, `set_android_volume`, or `run_command_profile`. Grant contracts are defined in `docs/CREATE_DIRECTORY_CAPABILITY_GRANTS.md`, `docs/COPY_FILE_CAPABILITY_GRANTS.md`, and `docs/WRITE_FILE_CAPABILITY_GRANTS.md`; filesystem contracts remain in the corresponding `docs/SAFE_ROOT_*` documents.

`run_command_profile` is supported only for the three fixed diagnostics of the exact running server binary. It accepts no raw command, program, argv, working directory, environment, stdin, timeout, or output-limit input. `set_android_volume` is supported only for the six fixed Termux:API streams, preview by default, and one exact single-use grant per live mutation with verification and recovery. Public directory-creation, copy, file-write, and volume-control library APIs are preview-only; their live preparation and execution are crate-private and reachable only through grant-aware transport. One internal registry assigns globally unique wire codes to directory (`1`), write (`2`), volume (`3`), and copy (`4`) request-grant families, and every ordered cross-family use fails without consuming the source grant. Broader Android platform control, shells, arbitrary command execution, global process inventory, arbitrary service inspection, service mutation/control, package management, network mutation, and unrelated high-impact controls are not supported runtime surfaces.
