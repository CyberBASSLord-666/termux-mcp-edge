...
The supported runtime line has five explicit compile-time postures:
...
The baseline staged tools remain limited to `runtime_status`, `platform_info`, `android_status`, `project_service_status`, dry-run-first `create_directory`, `list_directory`, `path_metadata`, `read_file`, bounded literal `search_text`, and dry-run-first `write_file`. Directory creation is descriptor-relative, creates one absent target with fixed mode `0700`, uses atomic no-replace publication, and requires explicit `dry_run: false`; it does not recurse or replace. Metadata and search are descriptor-relative and content-free; metadata omits host identifiers and permission internals under a 16 KiB response ceiling, while search never evaluates regex/shell syntax and has fixed traversal/file/byte/match/response ceilings. Separately built and explicitly enabled postures may add bounded read-only `android_battery_status`, `android_volume_status`, or `run_command_profile`. Provider contracts are defined in `docs/ANDROID_BATTERY_STATUS.md` and `docs/ANDROID_VOLUME_STATUS.md`; filesystem creation, metadata, and search are defined in `docs/SAFE_ROOT_DIRECTORY_CREATION.md`, `docs/SAFE_ROOT_PATH_METADATA.md`, and `docs/SAFE_ROOT_TEXT_SEARCH.md`; the command contract is defined in `docs/command-execution-gate.md`.

`run_command_profile` is supported only for the three fixed diagnostics of the exact running server binary. It accepts no raw command, program, argv, working directory, environment, stdin, timeout, or output-limit input. It uses a canonical safe-root cwd, cleared environment, null stdin, bounded streams, a hard deadline, process-group cleanup, zero-exit and UTF-8 requirements, and stable non-sensitive audit reasons. Android platform or volume control, shells, arbitrary command execution, global process inventory, arbitrary service inspection, service mutation/control, package management, network mutation, and high-impact controls are not supported runtime surfaces.
