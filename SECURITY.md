...
The separate `android-rish` feature is development-only, excluded from the seven governed postures and `full-suite`, and can expose only a diagnostic observation of the exact UID-2000 predicate token after static authentication plus private DEX validation. Stock rish does not make that observation proof of remote exit status, stream separation, typed Binder identity, or lifecycle. `full-suite` is compile-time inclusion, not a master permission. Battery, volume-status, volume-control, and fixed-command runtime flags remain independent; filesystem and volume mutations retain their separate default-disabled gates and exact-operation request grants. Raw Cargo `--all-features` is retained for development compatibility and is not a public release posture.

Release staging is also fail-closed. It reuses the exact qualified seven Android binaries without rebuilding, carries only sanitized physical evidence into GitHub, requires protected environment review, and has no tag or GitHub Release permission. Workflow-built and separately built native-device full-suite digests are recorded as distinct evidence lineages rather than assumed equal. See [`docs/PUBLIC_RELEASE.md`](docs/PUBLIC_RELEASE.md).

Release publication is a separate, narrower state machine. It requires a protected annotated tag and pre-created empty draft, attaches exactly seven binaries, seven checksum sidecars, `SHA256SUMS`, and the unchanged raw staging tar, then verifies all sixteen assets on a fresh read-only runner before a disjoint final environment may publish. Only the protected attachment and final jobs receive job-local `contents: write`; distinct environment-only Administration-read credentials can inspect the immutable-release policy but cannot mutate repository content. Stages and drafts are non-confidential release material, neither is installable, and publication is not complete until GitHub reports `immutable: true` and a public re-download reproduces every asset digest.
