//! Request-scoped authorization for `create_directory` mutation.
//!
...
//! normalized root-relative target, and the mutating posture. The runtime never
...
impl CreateDirectoryGrantTarget {
    pub(crate) fn from_normalized_components<'a>(
        root_device: u64,
        root_inode: u64,
        components: impl IntoIterator<Item = &'a [u8]>,
    ) -> Result<Self, CreateDirectoryGrantError> {
        let mut digest = Sha256::new();
        digest.update(TARGET_DIGEST_DOMAIN);
        let mut component_count = 0_u32;
        for component in components {
            let length = u32::try_from(component.len())
                .map_err(|_| CreateDirectoryGrantError::TargetInvalid)?;
            digest.update(length.to_be_bytes());
            digest.update(component);
            component_count = component_count
                .checked_add(1)
                .ok_or(CreateDirectoryGrantError::TargetInvalid)?;
        }
        if component_count == 0 {
            return Err(CreateDirectoryGrantError::TargetInvalid);
        }
        digest.update(component_count.to_be_bytes());
        Ok(Self {
            root_device,
            root_inode,
            target_digest: digest.finalize().into(),
        })
    }

    #[cfg(test)]
    fn test(root_device: u64, root_inode: u64, components: &[&[u8]]) -> Self {
        Self::from_normalized_components(root_device, root_inode, components.iter().copied())
            .expect("test target must be valid")
    }
}

impl fmt::Debug for CreateDirectoryGrantTarget {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CreateDirectoryGrantTarget")
...
pub enum CreateDirectoryGrantError {
...
impl CreateDirectoryGrantError {
...
impl fmt::Display for CreateDirectoryGrantError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(self.reason_code())
    }
}

impl std::error::Error for CreateDirectoryGrantError {}

#[derive(Clone)]
pub struct CreateDirectoryGrantAuthority {
    key_id: Arc<str>,
...
impl CreateDirectoryGrantAuthority {
    pub fn from_hex_key(
        ...
        Self::from_hex_key_with_capacity(
            ...
            MAX_CONSUMED_CREATE_DIRECTORY_GRANTS,
        )
    }
    ...
    fn from_hex_key_with_capacity(
        ...
        if !valid_key_id(&key_id)
            || key_hex.len() != CREATE_DIRECTORY_GRANT_KEY_HEX_BYTES
            || static_principal_secret.is_empty()
            || replay_capacity == 0
        {
            return Err(CreateDirectoryGrantError::ConfigurationInvalid);
        }
    ...
    pub fn issue_at(
        &self,
        session_id: &str,
        target: &CreateDirectoryGrantTarget,
        now_unix_seconds: u64,
    ) -> Result<String, CreateDirectoryGrantError> {
        let session_id = parse_canonical_session(session_id)?;
        let expires_unix_seconds = now_unix_seconds
            .checked_add(CREATE_DIRECTORY_GRANT_TTL_SECONDS)
        ...
        let grant = ParsedGrant {
            ...
            session_id,
            capability: RequestGrantCapability::CreateDirectory.code(),
    ...
    pub fn consume_at(
        ...
        target: &CreateDirectoryGrantTarget,
        now_unix_seconds: u64,
    ) -> Result<(), CreateDirectoryGrantError> {
        let token = token.ok_or(CreateDirectoryGrantError::Missing)?;
        ...
        if grant.principal_digest != self.principal_digest
            ...
            || grant.capability != RequestGrantCapability::CreateDirectory.code()
    ...
    fn encode_and_sign(&self, grant: &ParsedGrant) -> String {
        ...
        let signed = format!(
            "{CREATE_DIRECTORY_GRANT_VERSION}.{}.{}",
            self.key_id, payload_hex
        );
        let mut mac = HmacSha256::new_from_slice(self.key.as_ref())
            .expect("fixed-size HMAC key is always valid");
        mac.update(signed.as_bytes());
        let signature = mac.finalize().into_bytes();
        format!("{signed}.{}", encode_hex(&signature))
    }

    fn parse_and_verify(&self, token: &str) -> Result<ParsedGrant, CreateDirectoryGrantError> {
        if token.is_empty()
            || token.len() > MAX_CREATE_DIRECTORY_GRANT_HEADER_BYTES
            || !token.is_ascii()
        {
            return Err(CreateDirectoryGrantError::Malformed);
        }
        let mut segments = token.split('.');
        let version = segments
            .next()
            .ok_or(CreateDirectoryGrantError::Malformed)?;
        let key_id = segments
            .next()
            .ok_or(CreateDirectoryGrantError::Malformed)?;
        let payload_hex = segments
            .next()
            .ok_or(CreateDirectoryGrantError::Malformed)?;
        let signature_hex = segments
            .next()
            .ok_or(CreateDirectoryGrantError::Malformed)?;
        if segments.next().is_some() {
            return Err(CreateDirectoryGrantError::Malformed);
        }
        if version != CREATE_DIRECTORY_GRANT_VERSION {
            return Err(CreateDirectoryGrantError::UnknownVersion);
        }
        if key_id != self.key_id.as_ref() {
            return Err(CreateDirectoryGrantError::UnknownKey);
        }
        ...
        mac.verify_slice(&signature)
            .map_err(|_| CreateDirectoryGrantError::SignatureInvalid)?;
        decode_payload(&payload).ok_or(CreateDirectoryGrantError::Malformed)
    }
}

impl fmt::Debug for CreateDirectoryGrantAuthority {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CreateDirectoryGrantAuthority")
...
fn valid_key_id(key_id: &str) -> bool {
    !key_id.is_empty()
        && key_id.len() <= MAX_CREATE_DIRECTORY_GRANT_KEY_ID_BYTES
...
fn parse_canonical_session(
    session_id: &str,
) -> Result<[u8; SESSION_BYTES], CreateDirectoryGrantError> {
    let parsed =
        Uuid::parse_str(session_id).map_err(|_| CreateDirectoryGrantError::SessionInvalid)?;
    if parsed.to_string() != session_id {
        return Err(CreateDirectoryGrantError::SessionInvalid);
    }
    Ok(*parsed.as_bytes())
}
...
fn encode_payload(grant: &ParsedGrant) -> [u8; PAYLOAD_BYTES] {
    ...
    put(&mut payload, &mut offset, &grant.target_digest);
...
fn decode_payload(payload: &[u8; PAYLOAD_BYTES]) -> Option<ParsedGrant> {
    ...
    let target_digest = take_array(payload, &mut offset)?;
...
mod tests {
    ...
    fn target() -> CreateDirectoryGrantTarget {
        CreateDirectoryGrantTarget::test(41, 73, &[b"projects", b"new-directory"])
    }
    ...
    fn rejects_missing_malformed_unknown_and_invalid_signature_tokens() {
        ...
        for malformed in ["", "v1", "v1.primary-1.bad.bad", &"x".repeat(385)] {
            assert_eq!(
                authority
                    ...
                    .unwrap_err(),
                CreateDirectoryGrantError::Malformed
            );
        }
        ...
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::UnknownVersion
        );
        ...
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::UnknownKey
        );
        ...
        *last = if *last == b'0' { b'1' } else { b'0' };
        ...
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::SignatureInvalid
        );
    }
    ...
    fn rejects_every_mismatched_binding_without_detail() {
        ...
        let mismatches = [
            ...
            authority.consume_at(
                Some(&token),
                SESSION,
                &CreateDirectoryGrantTarget::test(42, 73, &[b"projects", b"new-directory"]),
                NOW,
            ),
            authority.consume_at(
                Some(&token),
                SESSION,
                &CreateDirectoryGrantTarget::test(41, 74, &[b"projects", b"new-directory"]),
                NOW,
            ),
            authority.consume_at(
                Some(&token),
                SESSION,
                &CreateDirectoryGrantTarget::test(41, 73, &[b"projects", b"other"]),
                NOW,
            ),
        ];
    ...
    fn rejects_expired_future_excessive_lifetime_and_clock_rollback() {
        ...
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::Expired
        );
        ...
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::FutureIssued
        );
        ...
        let excessive = resign_payload(&authority, &normal, |payload| {
            ...
            payload[expires_offset..].copy_from_slice(
                &(NOW + MAX_CREATE_DIRECTORY_GRANT_LIFETIME_SECONDS + 1).to_be_bytes(),
            );
        });
        assert_eq!(
            authority
                ...
                .unwrap_err(),
            CreateDirectoryGrantError::LifetimeExceeded
        );
    ...
    fn replay_storage_is_bounded_and_expired_entries_are_pruned() {
        ...
        authority
            .consume_at(
                ...
                SESSION,
                &target(),
                NOW + CREATE_DIRECTORY_GRANT_TTL_SECONDS,
            )
            .unwrap();
    }
