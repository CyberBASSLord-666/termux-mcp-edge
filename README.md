...
The feature includes `mcp-runtime`. A default build rejects the runtime flag, while a command build with the flag unset hides `run_command_profile` and denies direct calls without spawning. The package binary must also consume a one-time opaque `ServerCommandAuthority` that Cargo makes unobtainable when this crate is compiled as a dependency; every public embedding router is structurally command-disabled. At initialization, `current_exe` must be an executable regular file named exactly `termux-mcp-server`. Later profiles execute `/proc/self/exe`, binding each spawn to the already-running inode even if its installation path is renamed or replaced. Enabled MCP callers may choose only `server_version`, `server_help`, or `execution_boundary`. Fixed argv, the first canonical safe root as cwd, empty environment, null stdin, a five-second deadline, independent output ceilings, a two-permit non-queueing concurrency limit, process-group cleanup, zero-exit enforcement, and UTF-8-only bounded output remain mandatory. Crate-private profiles and execution types prevent forged raw execution; immutable supervisor maxima reject more than 5 seconds, 16 KiB stdout, or 4 KiB stderr before spawn; output capacity grows fallibly only as bytes arrive. Callers cannot supply a command, program, argv, path, environment, stdin, timeout, or limit.
