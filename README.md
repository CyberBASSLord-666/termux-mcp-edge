...
With all gates satisfied, `android_rish_status` accepts no arguments and runs only the fixed server-owned rish identity probe. The backend invokes `/system/bin/app_process64` with a private execution snapshot of the pinned DEX, fixed loader class, fixed Termux application identity, cleared environment, null stdin, fixed five-second and output bounds, and the fixed command `exec /system/bin/id -u`. Success requires exact stdout `2000\n` and empty stderr. The source DEX is canonicalized, owner/directory/mode/link/size/digest attested at startup, and retained by descriptor. Each call revalidates the private parent and source before copying, verifies that `/proc/self/fd/N` reopens to the digest-matched snapshot bytes (required for ART), then revalidates the source after the copy. Prefer a sealed memfd when path reopen works; otherwise use a private `O_TMPFILE` or exclusive named mode-`0400` copy. The sole non-queueing probe permit remains owned through blocking validation and cancellation-independent process-group termination and reaping, so cancelling a request cannot admit an overlapping probe.
