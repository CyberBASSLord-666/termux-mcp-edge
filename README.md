...
`write_file` preview also remains in the baseline registry while mutation is disabled by default. Explicit `dry_run:false` is necessary and not sufficient. Enable its independent gate only with static-token authentication and the private capability key pair:
...
The exact binary issues one 60-second grant with `--issue-write-file-grant`. The operator supplies the active canonical session, absolute safe-rooted target, and exact lowercase content SHA-256 through `MCP__CAPABILITY__SESSION_ID`, `MCP__CAPABILITY__WRITE_FILE_TARGET`, and `MCP__CAPABILITY__WRITE_FILE_CONTENT_SHA256`. The issuer infers create versus replace from the current descriptor-safe target posture. The runtime consumes the grant immediately before exclusive staging creation; preview never consumes it, and any later failure leaves it consumed. Writes are capped at 1 MiB and fixed mode `0600`. See [`docs/WRITE_FILE_CAPABILITY_GRANTS.md`](docs/WRITE_FILE_CAPABILITY_GRANTS.md) and [`docs/SAFE_ROOT_FILE_WRITES.md`](docs/SAFE_ROOT_FILE_WRITES.md).
...
- [Safe-root file-write contract](docs/SAFE_ROOT_FILE_WRITES.md)
- [Write-file capability grants](docs/WRITE_FILE_CAPABILITY_GRANTS.md)
