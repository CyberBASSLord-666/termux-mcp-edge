...
`trash_file` validates one existing safe-rooted file in preview while live removal is independently disabled. Production mutation requires `MCP__FILE__TRASH_FILE_MUTATION_ENABLED=true`, static authentication, the capability key pair, an active session, and one locally issued `--issue-trash-file-grant` token bound to `MCP__CAPABILITY__TRASH_FILE_TARGET`. The target must be a single-link regular file no larger than 1 MiB. Under lock-held identity and SHA-256 revalidation, one authorized operation atomically moves the exact inode into a mode-`0700` `.termux-mcp-trash-quarantine`; the public target becomes absent, recovery is retained, and no MCP surface exposes the target or randomized artifact path. There is no automatic purge or rollback. See [`docs/TRASH_FILE_CAPABILITY_GRANTS.md`](docs/TRASH_FILE_CAPABILITY_GRANTS.md) for issuance, transaction ordering, manual recovery, capacity, privacy, and same-UID operational requirements.
...
Validate the exact downloaded default, `mcp-runtime`, and `android-volume-control` Android candidates together through [`docs/RELEASE_CANDIDATE_VALIDATION.md`](docs/RELEASE_CANDIDATE_VALIDATION.md). Each workflow bundle includes an exact-source manifest and checksum sidecar; validator v8 reconciles all three with the supplied commit/run metadata, executes the seventeen-tool baseline including content-free path discovery, bounded file hashing, canonical binary reads, code-point-safe UTF-8 range pagination, request-granted file-write mutation, and reversible request-granted file trashing, proves the control posture remains hidden and inert by default without changing device audio, requires explicit confirmation for runtime/deployment phases, and emits only versioned sanitized JSON evidence.
