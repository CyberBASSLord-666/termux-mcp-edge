...
Validate the exact downloaded default, `mcp-runtime`, and `android-volume-control` Android candidates together through [`docs/RELEASE_CANDIDATE_VALIDATION.md`](docs/RELEASE_CANDIDATE_VALIDATION.md). Each workflow bundle includes an exact-source manifest and checksum sidecar; validator v3 reconciles all three with the supplied commit/run metadata, proves the control posture remains hidden and inert by default without changing device audio, requires explicit confirmation for runtime/deployment phases, and emits only versioned sanitized JSON evidence.
