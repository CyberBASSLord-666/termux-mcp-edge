...
The isolated `android-rish` feature is not an eighth governed artifact and is not a `full-suite` constituent. CI compiles, lints, and tests its default-disabled UID-2000 status foundation, but the official Termux container explicitly lacks an Android framework and cannot exercise Shizuku Binder permission or lifecycle. Adding a release-capable rish artifact requires a new evidence-schema and inventory revision plus exact physical AArch64 Android/Termux qualification; the existing seven-bundle, nine-workflow-artifact, twelve-qualification-member, and sixteen-public-asset contracts must not be reinterpreted as that evidence. See [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md).
