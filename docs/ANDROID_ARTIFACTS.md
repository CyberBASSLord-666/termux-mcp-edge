...
8. For the battery posture, disabled-default discovery plus enabled fixed-path, zero-argument, cleared-environment, normalized-output, overflow, provider-failure, audit, and no-device-control checks.
...
The Android workflow validates the third artifact with [`termux_battery_emulated_gate.sh`](../scripts/termux_battery_emulated_gate.sh) inside the pinned official Termux image on native ARM64. Its separate sanitized report conforms to [`android-battery-emulated-evidence-schema-v1.json`](android-battery-emulated-evidence-schema-v1.json). A passing feature-emulation report is required development evidence but is never, by itself, a physical-device release qualification.
