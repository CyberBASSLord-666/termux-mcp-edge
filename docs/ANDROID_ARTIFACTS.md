...
7. For the `mcp-runtime` posture, authenticated discovery and representative allowed and denied tool calls, including default-disabled directory mutation, exact-binary offline grant issuance, dry-run non-consumption, exact-target creation, replay denial, path metadata, and literal search.
