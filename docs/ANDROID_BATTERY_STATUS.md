...
The Android workflow builds a separate `android-battery-status` AArch64 artifact and executes it in the pinned official Termux image on a native ARM64 runner. That gate installs a temporary fixed-path API fixture and proves enabled and disabled discovery, fixed-working-directory/no-argument/environment-cleared execution, normalization/redaction, immediate endless-output rejection, stdout/stderr pipe-holder cleanup, client-cancellation cleanup, process-group termination, stable provider failure, and continued absence of device-control, command, and high-impact tools. Its strict, sanitized v3 evidence conforms to [`android-battery-emulated-evidence-schema-v3.json`](android-battery-emulated-evidence-schema-v3.json) and binds the pinned base-userland identity separately from the derived runtime-image identity. This automated gate does not claim battery, OEM thermal-management, mobile-radio, or Android background-process behavior on physical hardware; optional physical certification remains a separate evidence tier without requiring a long observation for every development PR or ordinary release.
