...
- allowed bounded `trash_file` previews and exact request-authorized recovery-retaining moves only after identity/content verification and durability confirmation
- denied trash requests, including invalid arguments, disabled mutation, safe-root/type/link/size/response rejection, every stable grant failure, target change, quarantine capacity/shape/lock failure, and internal failure
...
`trash_file` follows the same requested-mode rule under its independent gate. The worker owns its exactly-once terminal audit: pre-consumption cancellation, target change, capacity, and lock failures are denials without grant consumption; success records `safe_root_file_trashed_recovery_retained` only after the public name is absent, the exact inode/content is retained, and both parent and quarantine synchronization succeed. No event records the target or artifact path, identity, digest, bytes, grant, session, or recovery usage.
...
The `trash_file` result is path-, artifact-, content-, and digest-free under the same 16 KiB ceiling. Its fields are only `dryRun`, `sizeBytes`, `recoveryArtifactRetained`, `maxFileBytes`, and `maxResponseBytes`; preview reports no retained artifact and successful live mutation reports retention.
...
3. `trash_file` records an allowed preview only after bounded validation, one allowed live terminal decision only after exact recovery retention and durability, and stable disabled/grant/target/capacity/lock/cancellation failures without retaining target or artifact material.
...
19. Exact release-validator and native Termux device-smoke tests prove the trash and write gates, grant reason buckets, recovery-retention results, and bounded quarantine denials without serializing keys, grants, principal/session/JTI bindings, paths, content, digests, filesystem identities, or artifact names.
