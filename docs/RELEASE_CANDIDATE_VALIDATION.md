...
The phase starts each artifact directly on loopback, one at a time, using a private token and a unique validation child below `SAFE_ROOT`. Child environments are rebuilt from an explicit allowlist so ambient `MCP__*` settings cannot broaden the test. The validator creates one private random capability key, independently enables only the `mcp-runtime` candidate's directory and file-write mutation gates, invokes that exact binary's distinct offline issuers, and destroys both private grant files with its isolated workspace. Sharing the signing key never lets one capability's grant authorize the other.
...
- enabled file-write discovery/status and fixed public limits; missing, wrong-context, cross-capability, wrong-session/target/content/disposition binding, and replay denial; preview non-consumption; exact-binary mode-`0600` create and replace; create/replace mismatch; exact 1 MiB acceptance and plus-one rejection using private request files; and oversized actual-ID 16 KiB response preflight followed by successful same-grant reuse;
...
- disabled-posture write discovery/status and explicit-mutation denial in every unrelated optional capability posture;
...
Response bodies, safe-root paths, test file contents/digests, bearer tokens, capability keys/grants, and session identifiers stay in the private temporary workspace and are deleted. They are never copied into JSON evidence. A passing validator-v8 runtime result includes `request_scoped_single_use_grant_enforced`, `expanded_body_posture_verified`, `safe_root_file_create_replace_verified`, `request_scoped_single_use_write_grant_enforced`, `exact_write_file_byte_limit_verified`, `bounded_write_file_response_preflight_verified`, `safe_root_path_discovery_verified`, `safe_root_file_hash_verified`, `safe_root_binary_read_verified`, `safe_root_binary_range_read_verified`, `incompatible_volume_control_artifact_rejected`, `volume_control_hidden_while_disabled`, and `volume_control_disabled_call_rejected`.
