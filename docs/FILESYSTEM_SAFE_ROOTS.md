...
The recovery quarantine is capped at 32 regular artifacts and 32 MiB per target parent. Its nonblocking advisory lock coordinates cooperating runtime writers only. It is not a global disk limit or an isolation boundary from another process under the same Unix UID; such a peer can force contention or denial of service. Quiesce the service and all same-UID writers before inspecting or manually removing selected artifacts.
