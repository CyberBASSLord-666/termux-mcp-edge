...
## Secure Router Builder Validation

The default-feature and all-feature gates jointly validate the only public construction path:

1. An ordinary downstream path dependency compiles `McpRouterBuilder::new(...).build()` with validated auth, request-limit, transport-security, and safe-root inputs.
2. Compile probes prove raw/legacy router and state constructors, protection/capability groupings, command internals, and the builder's command-enablement setter are not downstream API.
3. Typed construction tests reject every safe-root error class; malformed listener declarations; unauthenticated policy paired with non-loopback declarations; every filesystem and Android-volume mutation authority paired with unauthenticated policy; requested but uncompiled capabilities; and optional-client initialization failure.
4. Integration tests send unauthenticated POST, GET, and DELETE requests through the one public builder and cover initialize/session allocation, `tools/list`, reads, fake capability-grant handling, and create/copy/write mutation attempts. Every path stops at authentication and creates no filesystem effect.
5. Oversized unauthenticated bodies prove authentication precedes early `Content-Length`, concurrency, timeout, streaming body limiting, extraction, Host/Origin validation, parsing, sessions, tools, grants, and mutation.
6. Explicit localhost development tests prove the builder rejects non-loopback declarations and middleware rejects missing or non-loopback `ConnectInfo`; only actual IPv4/IPv6 loopback peers pass.
7. Production startup tests and integration fixtures use the same builder instead of manually reproducing layers.
...
5. Create, copy, and write grant issuers bind their independently pinned root identity. Runtime target preparation and consumption compare against the running pin, including an explicit regression where a grant issued after replacement fails to authorize the original pinned root.
...
Live `create_directory`, `copy_file`, and `write_file` first require the one shared, fixed, fail-fast, non-queueing mutation-worker permit. The permit owns descriptor preparation and the complete blocking commit, so an HTTP timeout cannot release capacity while detached preparation continues. After preparation, the worker acquires one poison-fail-closed process-wide publication lock shared across every tool and router state, then revalidates the prepared absent, exact source/destination, or exact replace identity. Only then does the atomic cancellation/worker-ownership guard decide commit: cancellation first, including while waiting for the process lock, consumes no grant and mutates nothing; stale revalidation likewise preserves the grant for fresh preparation. Worker ownership first makes completion cancellation-independent and consumes the grant immediately before mutation. The process lock remains held through publication verification and durability sync. `copy_file` and `write_file` create randomized mode-`0600` staging inodes inside the fixed per-parent `.termux-mcp-write-quarantine` and verify descriptor, name, content, and identity before publication. Copy and write-create publish with `NOREPLACE` and retain no artifact; write-replace performs one identity-verified irreversible `EXCHANGE`. Replace leaves the displaced prior inode/content and its existing metadata under the randomized quarantine name; no automatic unlink, truncation, chmod, or swap-back follows capture. The write result reports `recoveryArtifactRetained:false` for preview/create and `true` for successful replace; copy results expose neither endpoint path, content, digest, nor staging name.
...
rm -f "$GRANT_FILE" "$COPY_GRANT_FILE" "$WRITE_GRANT_FILE" "$WRITE_CONTENT_FILE"
...
The exact-candidate release validator and Termux device-smoke gate must deterministically cover the write truth table above along with install/upgrade/rollback/uninstall recovery. Filesystem authority, grant authorization, and other deterministically testable development changes do not require a new arbitrary 60-minute monitoring window. Physical observation is required only when the release evidence classifier identifies battery, thermal, Android storage/mount, OEM process-management, or other device-only behavior not covered by exact-artifact host tests, emulated Termux, and focused native checks.
