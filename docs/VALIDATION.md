...
Trash validation is independent of create, copy, and write. With `MCP__FILE__TRASH_FILE_MUTATION_ENABLED=false`, prove discovery constrains `dry_run` to `true` and live calls return `trash_file_mutation_disabled` before path access. When enabled, use the exact-binary issuer and prove family `5`, principal/session/root/path/target-identity/size/ctime/link-count/SHA-256/recovery-posture/time/signature/replay bindings; preview non-consumption; exact 1 MiB acceptance; stale-target, capacity, quarantine-lock, response-preflight, cancellation, and concurrent single-use behavior; exact inode/content/mode retention; hidden exact/mixed-case namespace and aliases; hostile recovery-entry denial without cleanup; and path/artifact/content/digest/identity-private results and audits. See [`TRASH_FILE_CAPABILITY_GRANTS.md`](TRASH_FILE_CAPABILITY_GRANTS.md).
...
Validate reversible trash issuance separately with `MCP__FILE__TRASH_FILE_MUTATION_ENABLED=true` and the exact candidate:
...
Send the grant only on the matching explicit live call and remove it after the attempt. Verify the public target becomes absent, one exact recovery artifact is retained locally, no MCP response exposes either path, and replay is denied.
...
Live `create_directory`, `copy_file`, `trash_file`, and `write_file` first require the one shared, fixed, fail-fast, non-queueing mutation-worker permit. The permit owns descriptor preparation and the complete blocking commit, so an HTTP timeout cannot release capacity while detached preparation continues. After preparation, the worker acquires one poison-fail-closed process-wide publication lock shared across every tool and router state, then revalidates the prepared absent, exact source/destination, trash target, or replace identity. Only then does the atomic cancellation/worker-ownership guard decide commit: cancellation first, including while waiting for the process lock, consumes no grant and mutates nothing; stale revalidation likewise preserves the grant for fresh preparation. Worker ownership first makes completion cancellation-independent and consumes the grant immediately before mutation. The process lock remains held through publication verification and durability sync. `copy_file` and `write_file` create randomized mode-`0600` staging inodes inside `.termux-mcp-write-quarantine`; trash moves the exact original inode into `.termux-mcp-trash-quarantine` with `NOREPLACE`. Copy and write-create retain no artifact; write-replace performs one identity-verified irreversible `EXCHANGE`; trash always reports recovery retained after success. No automatic unlink, truncation, chmod, swap-back, or purge follows capture, and no result exposes an endpoint or artifact path.
