...
- discovery lists exactly fourteen baseline tools, plus only those battery, volume-status, volume-control, and fixed-command tools whose independent gates are active (fifteen with one through eighteen with all four);
