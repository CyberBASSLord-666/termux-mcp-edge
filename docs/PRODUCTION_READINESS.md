...
- discovery lists exactly seventeen baseline tools, plus only those battery, volume-status, volume-control, and fixed-command tools whose independent gates are active (eighteen with one through twenty-one with all four);
...
- filesystem tools remain safe-rooted and bounded; mutations remain dry-run-first; directory creation is additionally default-disabled, exact-target grant-gated, fixed-mode/no-replace/non-recursive, and single-use; `trash_file` is independently default-disabled, exact principal/session/root/path/identity/content/recovery-posture grant-gated, 1 MiB/16 KiB bounded, atomic no-replace moved into a private retained quarantine, and path/artifact-free; `write_file` is independently default-disabled, exact principal/session/root/target/content/disposition/old-identity grant-gated, 1 MiB, target-mode `0600`, 16 KiB content/path-free, single-use, create-`NOREPLACE` without retention, and irreversible replace-`EXCHANGE` with bounded displaced-object preservation; file copy is single-regular-file, 1 MiB, binary-safe, fixed-mode, content-private, and no-replace; path discovery is literal-basename-only, descriptor-relative, 8,192-entry/512-match/depth/response bounded, and content-free; hashing is single-regular-file, streaming SHA-256, 16 MiB, and digest/path/content-private; binary read is single-regular-file, canonical padded base64, 1 MiB raw, response-preflighted, and path/host-metadata-private; metadata is descriptor-classified/content-free; and text search is literal/content-free;
