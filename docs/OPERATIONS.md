...
- MCP transport restoration tracked in staged PRs, with Android platform API/control tools, process inspection, command execution, and high-impact controls still disabled.
...
The current staged discovery surface is limited to `runtime_status`, `platform_info`, `android_status`, `list_directory`, `read_file`, and `write_file`. `android_status` is read-only status metadata only. `service_status` is present only as internal project-owned data primitives and is not yet exposed through MCP. Android platform APIs/control tools, process inspection, shell fallback, command execution, and high-impact controls remain unavailable.
...
The `service_status` module is currently an internal, data-only primitive for the allowlisted `mcp_runtime` logical service. It must not be treated as MCP tool exposure. Process inventory, PID inspection, command lines, environment exposure, service mutation, shell fallback, and service-control actions remain disabled.

Further filesystem, Android/platform, service-status transport, command-capable, and high-impact tool work must remain gated behind staged PRs with explicit tests, documentation, and Security validation.
