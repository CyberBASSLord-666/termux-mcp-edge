...
6. `list_directory` — bounded safe-rooted listing.
7. `path_metadata` — bounded safe-rooted regular-file or directory metadata without content or host identifiers.
8. `read_file` — bounded safe-rooted UTF-8 reads.
9. `search_text` — bounded case-sensitive literal UTF-8 location search without content excerpts.
10. `write_file` — safe-rooted, payload-bounded, dry-run-first writes.

An `android-battery-status` binary with `MCP__ANDROID__BATTERY_STATUS_ENABLED=true` additionally exposes `android_battery_status` as the eleventh tool. It is disabled and hidden by default; see [`ANDROID_BATTERY_STATUS.md`](ANDROID_BATTERY_STATUS.md).

An `android-volume-status` binary with `MCP__ANDROID__VOLUME_STATUS_ENABLED=true` instead exposes `android_volume_status` as the eleventh tool. It is independently disabled and hidden by default, uses only the fixed zero-argument `termux-volume` status mode, and never authorizes volume mutation; see [`ANDROID_VOLUME_STATUS.md`](ANDROID_VOLUME_STATUS.md). An all-feature validation build can expose both provider tools when both runtime flags are explicitly enabled.

A `command-execution` binary with `MCP__COMMAND__ENABLED=true` exposes `run_command_profile` after the ten baseline tools. It offers only the exact `server_version`, `server_help`, and `execution_boundary` profiles of the current server binary. It remains hidden when disabled; see [`command-execution-gate.md`](command-execution-gate.md). An all-feature validation build exposes thirteen tools only when all three runtime flags are explicitly enabled.
