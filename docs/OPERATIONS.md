...
A `command-execution` binary with `MCP__COMMAND__ENABLED=true` exposes `run_command_profile` after the seventeen baseline tools. It offers only `server_version`, `server_help`, and `execution_boundary` against the attested already-loaded server image. Command enablement is a crate-private method available only to the package binary; the single public builder defaults disabled and exposes no enabling method. Initialization matches the exact-name no-follow candidate to `/proc/self/exe` by device/inode and retains the first safe root by no-follow directory descriptor; children spawn `/proc/self/exe` with cwd `/proc/self/fd/<fd>`, empty environment, null stdin, and immutable maxima of 5 seconds, 16 KiB stdout, and 4 KiB stderr. It remains hidden when disabled; see [`command-execution-gate.md`](command-execution-gate.md). The governed `full-suite` exposes twenty-one tools only when all four governed optional runtime flags are explicitly enabled.

An isolated `android-rish` development binary with static authentication, a pinned private DEX path/digest, and `MCP__ANDROID__RISH_ENABLED=true` exposes `android_rish_status` as one additional tool. It accepts no arguments and observes only exact `2000\n` bytes in exactly one local rish capture; it proves no remote exit/stream/Binder lifecycle property, and root, arbitrary shell, backend command input, raw output, and mutations remain unavailable. An `--all-features` development build can therefore expose twenty-two tools, but it is not the governed aggregate or a release artifact. See [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md).

The runtime does not expose Android platform control beyond exact request-authorized volume, an arbitrary shell or command runner, global process inventory, arbitrary service inspection, service mutation, package management, network mutation, or unrelated high-impact controls. The rish exact-token diagnostic does not broaden that statement.
