...
An isolated `android-rish` development binary with static authentication, a pinned private DEX path/digest, and `MCP__ANDROID__RISH_ENABLED=true` exposes `android_rish_status` as one additional tool. It accepts no arguments and proves only exact Android shell UID `2000`; root, arbitrary shell, backend command input, raw output, and mutations remain unavailable. An `--all-features` development build can therefore expose twenty-two tools, but it is not the governed aggregate or a release artifact. See [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md).

The runtime does not expose Android platform control beyond exact request-authorized volume, an arbitrary shell or command runner, global process inventory, arbitrary service inspection, service mutation, package management, network mutation, or unrelated high-impact controls. The rish identity probe does not broaden that statement.
