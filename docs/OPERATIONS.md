...
Internal Android/Termux status primitives are data-only and are not exposed as an MCP Android platform API/control tool. Android platform APIs, process inspection, shell or command execution, and high-impact controls remain disabled and must be restored only through later staged PRs with explicit tests, documentation, and gate review.
...
7. Verify staged MCP transport and tool discovery only when the binary is built with `--features mcp-runtime`.
8. Do not claim MCP production readiness until the staged transport and tool-call validation passes for the intended deployment.
