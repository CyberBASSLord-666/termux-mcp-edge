...
The emulated job starts only after the default, `mcp-runtime`, `android-battery-status`, `android-volume-status`, and `command-execution` Android build postures complete. It downloads the bundles produced by the same workflow run and verifies:
...
- `mcp-runtime` authentication, Host/Origin ordering, initialization, notification semantics, protocol/session headers, exact tool allowlist, representative allowed and denied calls, safe-root confinement, bounded literal text-search execution, symlink denial, request/response bounds, and session deletion;
