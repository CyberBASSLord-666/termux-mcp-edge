...
- Binding: static principal, canonical session UUID, capability, safe-root device/inode, normalized target digest, exact content digest, exact `create`/`replace` disposition, and—on replacement—the existing target device, inode, size, high-resolution ctime, and one-link count, plus mutating posture. The fixed 64-byte payload (128 lowercase hex) carries only a random JTI, keyed domain-separated operation binding, and issued/expiry timestamps; it does not serialize those binding inputs.
