...
This note records the required gate for the current filesystem stage and the continuing constraints for later write-capable, service-status, or higher-impact MCP runtime surfaces.
...
- `service_status` exists only as internal project-owned data primitives for the allowlisted `mcp_runtime` logical service; it is not yet exposed through MCP.
- Android platform API/control tools, process inspection, shell fallback, command execution, and high-impact controls remain unavailable.
...
The current `service_status` primitive is data-only and must not be treated as service-control readiness:

1. It may model only fixed, in-repo, project-owned logical service names.
2. The only current allowlisted service is `mcp_runtime`.
3. It must not enumerate arbitrary processes or inspect arbitrary PIDs.
4. It must not expose command lines, process environments, usernames, hostnames, ports, package lists, service unit files, logs, stdout, stderr, or broad filesystem state.
5. It must not start, stop, restart, kill, mutate, or shell out to control services.
6. Any MCP transport exposure for service status must be a separate current-base PR with exact-head CI/Security validation.
...
- No service-control surface without a separate authorization and mutation policy.
