...
- The implementation bypasses the mutable launcher script. It invokes the fixed Android system runtime `/system/bin/app_process64`, the fixed loader class `rikka.shizuku.shell.ShizukuShellLoader`, the fixed Termux application identity `com.termux`, and the pinned DEX through its inherited `/proc/self/fd` descriptor.
