...
An `android-rish` build may additionally expose `android_rish_status` only when `MCP__ANDROID__RISH_ENABLED=true`, static bearer authentication is active, and the paired DEX path and lowercase SHA-256 are valid. The configured canonical DEX must remain outside every filesystem safe root, inside an owner-only parent, and be a current-UID-owned, single-link, exact-mode `0400` regular file within the fixed size bound. Construction retains a no-follow descriptor and verifies its digest; every probe rechecks descriptor, path, parent, ownership, mode, identity, timestamps, size, link count, and digest, copies the exact bytes into a sealed anonymous descriptor, verifies its write/grow/shrink/seal locks and digest, and revalidates the source before launching. Only that immutable snapshot reaches the intended child. The backend uses only fixed `/system/bin/app_process64`, the fixed Shizuku loader class, fixed `com.termux` identity, a cleared two-variable environment, null stdin, five-second supervision, 1 KiB/4 KiB output ceilings, and one non-queueing permit. The permit spans blocking validation and independently owned cancellation cleanup through confirmed reaping. Success is only exact stdout `2000\n` with empty stderr. The tool accepts no arguments and reports root rejected, arbitrary shell false, and mutation readiness false. Paths, digests, descriptors, raw output, and OS errors are suppressed.

The rish posture is excluded from the governed Android artifact/evidence chain until a physical AArch64 Android/Termux device proves the real Shizuku Binder permission and lifecycle. Host and raw all-feature checks prove compilation and fail-closed logic only. See [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md).

Android controls beyond the request-authorized exact-stream volume capability, shell fallback, arbitrary command execution, global process inventory, arbitrary service inspection, service mutation/control, package management, network mutation, and unrelated high-impact controls remain disabled. The rish identity probe does not change that authority statement.
...
| `android_rish_status` | Disabled | Development-only `android-rish` build with static authentication, explicit runtime opt-in, pinned DEX attestation, and exact UID-2000 response; no shell, Android action, or mutation |
