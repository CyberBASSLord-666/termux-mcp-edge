...
The quarantine lock is advisory and nonblocking. It coordinates cooperating runtime writers but cannot constrain a process running under the same Unix UID; such a process can cause contention, mutate names, or exhaust storage elsewhere. The 32-entry/32-MiB ceiling is per target parent, not a global disk bound. Operators must stop the service and other same-UID writers before inspecting or manually removing selected recovery artifacts.
