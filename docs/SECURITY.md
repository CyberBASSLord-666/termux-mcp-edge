...
A `command-execution` build may additionally expose `run_command_profile` only when `MCP__COMMAND__ENABLED=true`, the Cargo primary-package binary consumes its opaque `ServerCommandAuthority`, and executable initialization succeeds. Dependency builds cannot acquire that authority, and every public embedding router is command-disabled. Initialization requires `current_exe` to be an executable regular file with exact basename `termux-mcp-server`; profiles then launch `/proc/self/exe`, binding execution to the already-running inode across installation-path replacement. Project-owned argv selects version, help, or boundary self-check output. The working directory is an anchored safe root; the inherited environment is empty; stdin is null; time, both output streams, concurrency, process groups, cancellation cleanup, and direct-child reaping are bounded. Independent hard maxima reject more than 5 seconds, 16 KiB stdout, or 4 KiB stderr before spawn, while capacity grows fallibly only for bytes actually read. The request and public Rust API cannot select a program, forge or inspect a resolved profile, reach raw argv, or override cwd, environment, stdin, timeout, or limits. Failures suppress child output and use stable reasons.
...
Bearer authentication does not by itself authorize `create_directory` or `write_file` mutation. Each has an independent default-disabled runtime gate, while both require a paired 32-byte HMAC key configuration and one 60-second, single-use `MCP-Capability-Grant`. Directory grants bind the principal, active session, anchored safe root, normalized target, and mutating posture. Write grants additionally bind the exact UTF-8 content digest, `create` or `replace` disposition, and the exact existing device, inode, size, high-resolution ctime, and link count for replacement. A write payload is only 65 opaque bytes (130 lowercase hex): random JTI, the signed write-family byte, keyed operation binding, and issued/expiry timestamps, never the raw binding material. The runtime consumes the JTI immediately before its first filesystem mutation attempt and retains consumption after downstream failure. Grants are capability-distinct, header-only, and must never appear in arguments, URLs, responses, logs, audit labels, tickets, or screenshots. See [`CREATE_DIRECTORY_CAPABILITY_GRANTS.md`](CREATE_DIRECTORY_CAPABILITY_GRANTS.md) and [`WRITE_FILE_CAPABILITY_GRANTS.md`](WRITE_FILE_CAPABILITY_GRANTS.md).
...
| `run_command_profile` | Disabled | Available only to the primary package server in the `command-execution` build with explicit runtime opt-in and successful executable initialization; three fixed read-only diagnostics |
