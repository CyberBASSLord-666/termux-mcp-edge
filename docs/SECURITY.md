...
The additional `android-rish` feature is an isolated development posture, not an eighth governed release posture and not a constituent of `full-suite`. It adds only a default-disabled diagnostic that observes the exact UID-2000 predicate token in one local capture. It does not prove remote exit status, stream separation, typed Binder identity, or lifecycle. `full-suite` adds compiled code, not authority. It does not enable a constituent runtime flag, satisfy static authentication, or replace any filesystem or volume request grant. Raw Cargo `--all-features` remains a development compatibility posture and is not the named public aggregate artifact.
...
A second, independently default-disabled setting enables a first-stage MCP `2026-07-28` stateless subset without changing the legacy path. Modern requests are POST-only and JSON-only, carry their protocol version and client capabilities in `params._meta`, and mirror the method and tool name in routing headers. The subset exposes discovery, listing, read-only calls, and mutation previews only. It rejects every modern capability-grant header and explicit `dry_run:false` before dispatch, so live mutation remains bound to initialized legacy sessions and v1 grants.

In static-token mode, the complete `/mcp` route requires `Authorization: Bearer <configured-token>` before request resource limits, transport validation, JSON-RPC parsing, tool discovery, or tool invocation. Missing, malformed, oversized, or incorrect credentials are rejected with HTTP 401 and a non-sensitive response. The only authentication bypass is explicit unauthenticated localhost-only development mode. Startup validates an actually bound native IPv4 or IPv6 loopback listener, and the authentication middleware separately requires request-time `ConnectInfo<McpConnectionInfo>` derived from the accepted TCP stream. Its peer must be native IPv4 or IPv6 loopback and its local address must exactly match the listener validated by the builder. IPv4-mapped IPv6 literals, wildcard listeners, missing connection metadata, non-loopback peers, and listener substitution fail closed before request limits; `Host`, `Origin`, and forwarding headers cannot satisfy either socket check.
...
An `android-rish` build may additionally expose `android_rish_status` only when `MCP__ANDROID__RISH_ENABLED=true`, static bearer authentication is active, and the paired DEX path and lowercase SHA-256 are valid. The configured canonical DEX must remain outside every filesystem safe root, inside an owner-only parent, and be a current-UID-owned, single-link, exact-mode `0400` regular file within the fixed size bound. Construction retains a no-follow descriptor and verifies its digest; every probe rechecks descriptor, path, parent, ownership, mode, identity, timestamps, size, link count, and digest, copies the exact bytes into a fully sealed memfd, verifies its write/grow/shrink/seal locks, digest, and fresh ART-style path reopen, and revalidates the source before launching. No mutable tmpfile or named-file fallback is executable. The backend uses only fixed `/system/bin/app_process64`, the fixed Shizuku loader class, fixed `com.termux` identity, fixed rish variables plus a closed ART environment allowlist, null stdin, five-second supervision, 1 KiB/4 KiB local-capture ceilings, and one non-queueing permit. The permit spans blocking validation and independently owned local cancellation cleanup through confirmed reaping. Success is only exact `2000\n` bytes in exactly one local capture with the other empty. Stock rish does not make that a trustworthy remote-exit, stream-separation, Binder-identity, or lifecycle proof. The tool accepts no arguments and reports root rejected, arbitrary shell false, and mutation readiness false. Paths, digests, descriptors, raw output, and OS errors are suppressed.

The rish posture is excluded from the governed Android artifact/evidence chain until a typed lifecycle-authoritative bridge is implemented and a new reviewed evidence-schema/inventory revision binds that bridge to exact physical AArch64 Android/Termux qualification. Existing v1 physical evidence and host/raw-all-feature checks establish only the diagnostic and fail-closed logic; they cannot elevate it into release or production authority. See [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md).

Android controls beyond the request-authorized exact-stream volume capability, shell fallback, arbitrary command execution, global process inventory, arbitrary service inspection, service mutation/control, package management, network mutation, and unrelated high-impact controls remain disabled. The rish exact-token diagnostic does not change that authority statement.
...
| `POST`, `GET`, `DELETE /mcp` stable transport | Disabled | Bearer-authenticated and resource-bounded except explicit loopback development mode; legacy `2025-11-25` remains the default, with a separately gated stateless `2026-07-28` read/preview subset |
...
| `android_rish_status` | Disabled | Development-only `android-rish` build with static authentication, explicit runtime opt-in, pinned DEX validation, and exact UID-2000 local token observation; no shell, Android action, or mutation |
...
When `MCP__TRANSPORT__STATELESS_2026_07_28_ENABLED=true`, each modern POST instead requires `MCP-Protocol-Version: 2026-07-28`, a matching `params._meta.io.modelcontextprotocol/protocolVersion`, an object-valued `params._meta.io.modelcontextprotocol/clientCapabilities`, and an exact body-mirroring `Mcp-Method`. `Mcp-Name` is additionally required for `tools/call` and must equal `params.name`. Client information is optional and never substitutes for bearer authentication. Modern requests mint no session, return JSON only, and expose only `server/discover`, `tools/list`, and read-only or preview-only `tools/call`. Modern GET and DELETE return 405.

The modern path rejects `MCP-Capability-Grant` on every request with a non-sensitive invalid-parameters response, before dispatch. It also rejects explicit `dry_run:false` for every mutation-capable tool before provider, path, worker, or grant access. Omitted or true `dry_run` remains preview-only, and modern discovery fixes those schemas to `true`. Existing live mutation authority is deliberately not portable: all five mutation families remain legacy-session v1 only.
...
- `MCP__TRANSPORT__STATELESS_2026_07_28_ENABLED`: default `false`, strict boolean opt-in for the modern read/preview subset.
...
Version tags never rebuild Android release candidates. The protected staging workflow downloads the exact first-attempt successful current-main Android artifacts by immutable artifact ID, treats every server digest mismatch as an error, revalidates all seven closed manifests plus the four retained-runtime members and evidence chains, and copies governed bytes unchanged into the stage. It has only `actions: read` and `contents: read`; it cannot create a tag, GitHub Release, package, deployment, or OIDC identity. Its Actions artifact and dispatch inputs are not confidential in this public repository, so only sanitized evidence may cross this boundary.

Ordinary release qualification crosses into staging only through the exact upstream `official_termux_native_automated_v1` envelope, its digest-bound component reports, and its retained runtime archive/package lock/snapshot/offline replay. No dispatch input can select a qualification class or inject physical evidence. Every release-eligible automated envelope, staging manifest, and retained publication identity record states `physicalDeviceObserved:false`, `androidFrameworkObserved:false`, `sustainedPhysicalSoak:false`, and `physicalCertification:"not_run"`; applicable runtime records additionally require `rebuildReproducibilityClaim:false`. The assembler rejects physical minutes, equivalent minutes, time-dilation claims, duplicate JSON keys, stale policy/scenario digests, mixed runs/artifacts, runtime byte or identity mismatches, networked replay, and fixture evidence. Optional physical certification remains a separate historical-compatible tier and cannot broaden the automated envelope. Protected staging requires the pre-created `release-qualification` environment and repeats every check after approval. A missing or unprotected environment, moved `main`, rerun, expired artifact, evidence mismatch, or failed cleanup must block a qualified stage. A post-upload identity failure can leave an unqualified Actions artifact until administrative deletion or expiry because the read-only workflow has no delete authority. [`PUBLIC_RELEASE.md`](PUBLIC_RELEASE.md) defines the complete contract.

Automated release qualification proves the exact artifacts under the digest-pinned official Termux userland on native ARM64, including deterministic Android-provider simulation and isolated deployment recovery. It does not certify physical-device, OEM, battery-aging, thermal-soak, radio, Doze, or Android-framework behavior. The retained runtime archive is replayed offline and carries `rebuildReproducibilityClaim:false`; it does not claim future rebuild reproducibility.

Automated evidence files are assembled under caller-owned mode-`0700` output parents. Their shared commit helper opens the private source without following links, verifies mode, ownership, and SHA-256 through the held descriptor, and atomically hard-links that exact inode without replacing an existing public name. Failure cleanup never deletes the public path. This closes pre-commit pathname substitution and hard-link collision within the cooperating workflow model; it cannot prevent an arbitrary process under the same Unix UID from persistently mutating files after commit, so the qualification runner must remain isolated from unrelated same-UID writers.

Publication never extends the staging workflow's authority. The publisher requires the exact staging workflow run/artifact ID and server digest, a separately created protected annotated tag, and a pre-created exact-tag draft with zero assets. The `release-production` attachment job and `release-final` publication job have disjoint reviewers and distinct guard values. Each receives job-local `contents: write`; all preflight, independent verification, and public-proof jobs remain read-only. Their separate environment-only policy credentials are restricted to repository Administration-read and are used only to require release immutability to be enabled. They are never used as Release credentials. Because GitHub exposes no atomic precondition joining the last draft/assets read to the publish PATCH, final approval additionally requires the environment-only exclusive-mutation-freeze guard and a real bounded freeze of every other Release writer, repository-settings writer who could change immutable-release policy, tag or ruleset writer who could change candidate protection, `main` update, and relevant workflow rerun. API polling proves latest-observed state only; it does not replace that operational exclusion.

The attachment job binds the deterministic provenance-derived release body and uploads exactly seven binaries, seven `.sha256` sidecars, `SHA256SUMS`, and the unchanged raw staging tar after revalidating the four runtime members inside it. The runtime files remain tar-only and the receipt remains private. It cannot create or publish a Release, create or move a tag, change the title or prerelease state, delete a partial draft, or delete or replace an asset. GitHub does not make its validating GET and following body PATCH or asset POST atomic; a concurrent change can only stop the workflow and leave an unpublished, non-resumable partial draft, never trigger automatic deletion or repair. A fresh runner re-downloads all sixteen draft assets by server ID and proves byte equality against a separate exact-stage download before final approval is available. After the disjoint final reviewer approves, the final job repeats policy, tag, source, draft, asset-ID, server-digest, and byte checks and may only publish that draft. Completion requires `immutable: true` and unauthenticated public re-download proof for every governed asset. A partial or mismatched draft remains unpublished and is never auto-deleted; a post-publication proof failure is handled as a release incident and later corrected version, not mutable cleanup.

The stage, draft, and public assets contain only release-intended bytes and sanitized evidence and must all be treated as non-confidential. GitHub-generated source archives are outside the governed sixteen-asset allowlist. No workflow may claim a tag is GitHub-immutable before the associated immutable Release is published, or claim independent human approval unless the two environment reviewer sets are actually disjoint.
