...
Bearer authentication does not by itself authorize `create_directory`, `copy_file`, `trash_file`, or `write_file` mutation. Each has an independent default-disabled runtime gate and requires a paired 32-byte HMAC key configuration plus one 60-second, single-use `MCP-Capability-Grant`. Directory grants bind the principal, active session, anchored safe root, normalized target, and mutating posture. Copy grants bind both anchored roots and normalized paths, exact single-link source identity/size/high-resolution ctime/SHA-256, absent destination, and no-replace posture. Trash grants bind one anchored root/path, exact single-link target identity and SHA-256 content, and recovery-retaining posture. Write grants additionally bind the exact UTF-8 content digest, `create` or `replace` disposition, and exact existing identity for replacement. Each payload is only 65 opaque bytes (130 lowercase hex): random JTI, its distinct signed family byte, keyed operation binding, and issued/expiry timestamps, never raw binding material. The runtime consumes the JTI immediately before its first filesystem mutation attempt and retains consumption after downstream failure. Grants are capability-distinct, header-only, and must never appear in arguments, URLs, responses, logs, audit labels, tickets, or screenshots. See [`CREATE_DIRECTORY_CAPABILITY_GRANTS.md`](CREATE_DIRECTORY_CAPABILITY_GRANTS.md), [`COPY_FILE_CAPABILITY_GRANTS.md`](COPY_FILE_CAPABILITY_GRANTS.md), [`TRASH_FILE_CAPABILITY_GRANTS.md`](TRASH_FILE_CAPABILITY_GRANTS.md), and [`WRITE_FILE_CAPABILITY_GRANTS.md`](WRITE_FILE_CAPABILITY_GRANTS.md).
...
| `trash_file` | Disabled | Reversible preview-first removal of one single-link regular file up to 1 MiB; live mode requires an identity/content-bound grant and retains the exact inode in a private bounded quarantine |
...
`create_directory`, `copy_file`, `trash_file`, `find_paths`, `hash_file`, `list_directory`, `path_metadata`, `read_binary_file`, `read_binary_range`, `read_file`, `read_text_range`, `search_text`, and `write_file` are response or payload bounded. Directory, copy, trash, and write default to preview. Each live mutation requires its independent runtime gate and exact request grant; `dry_run:false` alone is never authorization. One process-global create/copy/trash/write publication lock covers lock-held posture revalidation, cancellation ownership, grant consumption, mutation, verification, and durability. Directory creation publishes fixed mode `0700` without replacement. Copy accepts one exact single-link source up to 1 MiB, revalidates its identity and SHA-256 plus destination absence, stages fixed mode `0600` in the hidden write quarantine, and publishes with `NOREPLACE`. Trash accepts one exact single-link target up to 1 MiB, revalidates its identity and SHA-256, and moves the inode with `NOREPLACE` into a separate mode-`0700` recovery quarantine. Write accepts at most 1 MiB of UTF-8; create uses `NOREPLACE`, while replacement accepts one exact single-link prior target and performs one irreversible `EXCHANGE` that preserves the displaced inode/content in the write quarantine. Both recovery namespaces are inaccessible through every MCP filesystem operation and capped at 32 regular artifacts and 32 MiB per target parent. Copy, trash, and write preflight their complete 16 KiB responses before path access and return no endpoint path, content, digest, or staging name; mutation results expose only bounded posture metadata.
...
The quarantine locks are advisory and nonblocking. They coordinate cooperating runtime writers but cannot constrain a process running under the same Unix UID; such a process can cause contention, mutate names, or exhaust storage elsewhere. Each 32-entry/32-MiB ceiling is per target parent, not a global disk bound. Operators must stop the service and other same-UID writers before inspecting, restoring, or manually removing selected recovery artifacts. Live filesystem mutation must also use safe roots under the service's exclusive operational ownership; independent writers must remain inactive while the `create_directory`, `copy_file`, `trash_file`, or `write_file` mutation gates are enabled.
