...
Bearer authentication does not by itself authorize `create_directory`, `copy_file`, or `write_file` mutation. Each has an independent default-disabled runtime gate and requires a paired 32-byte HMAC key configuration plus one 60-second, single-use `MCP-Capability-Grant`. Directory grants bind the principal, active session, anchored safe root, normalized target, and mutating posture. Copy grants bind both anchored roots and normalized paths, exact single-link source identity/size/high-resolution ctime/SHA-256, absent destination, and no-replace posture. Write grants additionally bind the exact UTF-8 content digest, `create` or `replace` disposition, and exact existing identity for replacement. Each payload is only 65 opaque bytes (130 lowercase hex): random JTI, its distinct signed family byte, keyed operation binding, and issued/expiry timestamps, never raw binding material. The runtime consumes the JTI immediately before its first filesystem mutation attempt and retains consumption after downstream failure. Grants are capability-distinct, header-only, and must never appear in arguments, URLs, responses, logs, audit labels, tickets, or screenshots. See [`CREATE_DIRECTORY_CAPABILITY_GRANTS.md`](CREATE_DIRECTORY_CAPABILITY_GRANTS.md), [`COPY_FILE_CAPABILITY_GRANTS.md`](COPY_FILE_CAPABILITY_GRANTS.md), and [`WRITE_FILE_CAPABILITY_GRANTS.md`](WRITE_FILE_CAPABILITY_GRANTS.md).
...
Validated construction lexically normalizes every configured safe root, walks it component by component from `/` with `O_PATH | O_NOFOLLOW`, rejects missing, non-directory, symbolic-link, parent-traversing, reserved, and filesystem-root inputs, and retains the resulting descriptor plus device/inode identity for the process lifetime. Each live create, copy, find, hash, list, metadata, read, search, and write derives a fresh directory handle from that pinned authority and verifies its identity with `fstat` before resolving descendants with descriptor-relative `openat` plus `O_NOFOLLOW`; the configured root pathname is never reopened. Directory enumeration, source reads, hashing, staging, publication, cleanup, and metadata lookup remain relative to held descriptors. Path discovery classifies children without following links, traverses only verified directories, skips invalid-UTF-8 names and special objects, and never reads file content. Copy additionally verifies source identity and size around its bounded read and verifies both held and published destination identities. Hashing and both binary-read tools verify that the opened regular descriptor matches the pre-open no-follow device/inode observation. Range reads also retain the initial descriptor size and reject a size change detected after the bounded seek/read. A concurrent directory/symlink exchange can therefore produce a bounded failure or continue against an already-open safe directory or file capability, but cannot redirect the operation through the replacement symlink. Deterministic adversarial tests cover swaps before and after descriptors are opened.
...
Broad shared-storage roots such as `/storage/emulated/0` and `/sdcard` are not defaults. Empty safe-root lists or entries, relative roots, parent traversal, missing components, non-directories, symbolic links, reserved namespaces, and filesystem root `/` are rejected during validated construction. Safe-root entries are not trimmed: whitespace is path data and a value that becomes relative because of leading whitespace fails closed. Renaming or replacing a configured root or any ancestor after startup cannot redirect authority: the running process continues against its pinned original inode until exit. Path replacement is therefore neither activation nor revocation; operators must stop the service, change the real directory/configuration, and start a newly validated process. Offline issuers and runtime consumption independently bind grants to the pinned root identity, so an issuer that observes a replacement root cannot mint a grant accepted by an older process pinned to the original root. Construction errors, `FileSystemTools` debug output, and aggregate audits expose neither configured root paths nor descriptor metadata.

`create_directory`, `copy_file`, `find_paths`, `hash_file`, `list_directory`, `path_metadata`, `read_binary_file`, `read_binary_range`, `read_file`, `read_text_range`, `search_text`, and `write_file` are response or payload bounded. Directory, copy, and write default to preview. Each live mutation requires its independent runtime gate and exact request grant; `dry_run:false` alone is never authorization. One process-global create/copy/write publication lock covers lock-held posture revalidation, cancellation ownership, grant consumption, mutation, verification, and durability. Directory creation publishes fixed mode `0700` without replacement. Copy accepts one exact single-link source up to 1 MiB, revalidates its identity and SHA-256 plus destination absence, stages fixed mode `0600` in the hidden quarantine, and publishes with `NOREPLACE`. Write accepts at most 1 MiB of UTF-8; create uses `NOREPLACE`, while replacement accepts one exact single-link prior target and performs one irreversible `EXCHANGE` that preserves the displaced inode/content in the quarantine. The quarantine is inaccessible through every MCP filesystem operation and capped at 32 regular artifacts and 32 MiB per target parent. Copy and write preflight their complete 16 KiB responses before path access and return no endpoint path, content, digest, or staging name; write additionally reports only bounded disposition and recovery-retention posture.
...
The quarantine lock is advisory and nonblocking. It coordinates cooperating runtime writers but cannot constrain a process running under the same Unix UID; such a process can cause contention, mutate names, or exhaust storage elsewhere. The 32-entry/32-MiB ceiling is per target parent, not a global disk bound. Operators must stop the service and other same-UID writers before inspecting or manually removing selected recovery artifacts. Live filesystem mutation must also use safe roots under the service's exclusive operational ownership; independent writers must remain inactive while the `create_directory`, `copy_file`, or `write_file` mutation gates are enabled.
