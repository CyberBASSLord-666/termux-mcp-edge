...
The `full-suite` feature only makes the four governed optional release
implementations available to the package build. It does not call any builder
opt-in, enable a runtime flag, create a mutation authority, or satisfy a request
grant. A full-suite process therefore has the same 17-tool discovery baseline
until its four optional runtime gates are enabled independently; with all four
enabled it has exactly 21 tools. Raw Cargo `--all-features` remains a
development compatibility posture, not an embedding or public artifact
contract.
