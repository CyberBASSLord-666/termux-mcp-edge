...
- An authenticated `tools/list` call includes the fifteen baseline tools. Battery, volume-status, volume-control, and fixed-command tools are absent by default; each appears only in its explicitly enabled posture. An all-feature test build has nineteen tools only when all four runtime flags are enabled.
