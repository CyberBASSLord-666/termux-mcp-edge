...
- An authenticated `tools/list` call includes the seventeen baseline tools. Battery, volume-status, volume-control, and fixed-command tools are absent by default; each appears only in its explicitly enabled posture. An all-feature test build has twenty-one tools only when all four runtime flags are enabled.
...
- With any directory, file-copy, reversible-file-trash, or file-write mutation gate disabled, the corresponding `create_directory`, `copy_file`, `trash_file`, or `write_file` schema constrains `dry_run` to `true`. With one enabled, only that tool's constraint is absent and its description names the exact header-grant requirement; no posture exposes an issuer tool.
...
- `trashFileMutationEnabled`, `trashFileMode`, grant/header/TTL/binding fields, 1 MiB target limit, 16 KiB response limit, 32-artifact/32-MiB recovery bounds, and path/artifact-free response posture report only fixed public truth.
...
- `trash_file` preview validates one exact single-link regular file up to 1 MiB without mutation, recovery-state creation, or grant consumption. Disabled live calls fail before path access. With the gate enabled, prove exact-binary `--issue-trash-file-grant`, family/principal/session/root/path/identity/content/recovery-posture binding, stale/capacity/lock/cancellation/replay/concurrent denial, one exact retained inode, hidden exact/mixed-case recovery namespace, hostile-entry fail-closed behavior, and path/artifact-free results/audits; see [`TRASH_FILE_CAPABILITY_GRANTS.md`](TRASH_FILE_CAPABILITY_GRANTS.md).
...
The canonical release validator and Termux device-smoke gate must repeat this write truth table against the exact candidate artifact: disabled discovery/direct-call denial; exact-binary issuance from private content; authorized create and replace; content/disposition/old-identity mismatch; replay denial; exact 1 MiB, mode `0600`, 16 KiB content-free result including `recoveryArtifactRetained`; `NOREPLACE` create without retention; irreversible replace with one bounded preserved recovery artifact; reserved-namespace isolation and capacity failure; no sensitive evidence fields; and preserved install/upgrade/rollback/uninstall recovery. These deterministic checks do not require an additional long idle observation merely because the write gate changed.
