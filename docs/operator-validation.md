...
- `create_directory`, `copy_file`, and `write_file` remain dry-run by default. All three mutations have independent default-disabled gates and request-grant bindings; `dry_run:false` alone must fail for each one.
...
- With any directory, file-copy, or file-write mutation gate disabled, the corresponding `create_directory`, `copy_file`, or `write_file` schema constrains `dry_run` to `true`. With one enabled, only that tool's constraint is absent and its description names the exact header-grant requirement; no posture exposes an issuer tool.
...
- Filesystem tools can escape configured safe roots; any mutation occurs without its explicit posture; or `create_directory`/`copy_file`/`write_file` mutation occurs without its own enabled gate and exact single-use request grant.
