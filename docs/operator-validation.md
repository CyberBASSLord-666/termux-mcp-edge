...
- `create_directory`, `copy_file`, and `write_file` remain dry-run by default. Directory and file-write mutation have independent default-disabled gates and independent request-grant bindings; `dry_run:false` alone must fail for either. File copy retains its explicit posture and safe-root controls.
...
8. Leave `MCP__FILE__CREATE_DIRECTORY_MUTATION_ENABLED=false`, `MCP__FILE__WRITE_MUTATION_ENABLED=false`, and `MCP__ANDROID__VOLUME_CONTROL_ENABLED=false` unless their exact mutations are operationally required. Each enabled gate requires static-token auth and a paired lowercase `MCP__CAPABILITY__KEY_ID` plus 64-lowercase-hex `MCP__CAPABILITY__HMAC_KEY_HEX`; keep both secrets owner-readable and out of transcripts. Enabling one gate does not enable either of the others.
...
- With the directory or file-write mutation gate disabled, the corresponding `create_directory` or `write_file` schema constrains `dry_run` to `true`. With one enabled, only that constraint is absent and its description names the exact header-grant requirement; no posture exposes an issuer tool.
...
- Filesystem tools can escape configured safe roots; any mutation occurs without its explicit posture; or `create_directory`/`write_file` mutation occurs without its own enabled gate and exact single-use request grant.
