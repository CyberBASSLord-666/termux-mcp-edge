...
- An authenticated `tools/list` call includes the thirteen baseline tools. Battery, volume-status, volume-control, and fixed-command tools are absent by default; each appears only in its explicitly enabled posture. An all-feature test build has seventeen tools only when all four runtime flags are enabled.
