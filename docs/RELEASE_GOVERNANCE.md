...
9. A separate first-attempt read-only `workflow_run` qualifier consumes the successful first-attempt Android run's frozen components, independently revalidates the exact CI, Security, Android, artifact, and current-`main` identities, and emits the closed `official_termux_native_automated_v1` envelope.
...
[`PUBLIC_RELEASE.md`](PUBLIC_RELEASE.md) defines the mandatory protected staging lane. Its manual workflow has only `actions: read` and `contents: read`, requires exact current-main source and first-attempt upstream run identity, independently discovers the exact-title separate qualifier run for that Android run and commit, and repeats every source, run, artifact, and evidence check after approval by the pre-created `release-qualification` environment. It has no physical-evidence dispatch input, caller-selected qualifier run, or caller-selected qualification class. Its temporary Actions artifact is not confidential in this public repository.
...
6. Require exactly nine 30-day Android artifacts: seven posture-specific bundles, one seven-report component artifact, and one three-file retained-runtime snapshot artifact. Verify names, manifests, checksum sidecars, executable identity, sizes, and SHA-256 checksums.
7. Retain aggregate schema/gate-v4 evidence that binds the exact workflow full-suite digest/manifest, 17/18/21 truth table, independent runtime gates, all four disabled filesystem dispatches, explicit covered/not-covered claims, and every posture's exact-source native ARM64 official-Termux evidence.
8. Require classifier v3 to route `automated_release_qualification` and the Android run to freeze the seven component reports and three retained-runtime source members without a release-eligibility conclusion. Then require the separate first-attempt read-only qualifier to revalidate that Android run, its exact CI/Security companions, current `main`, policy, scenarios, rootfs/userland, linker, seven artifact/manifest digests, seven component-evidence digests, runtime archive/package lock/snapshot, and no-network replay before assembling one exact twelve-member qualification artifact.
9. Run protected release staging from the exact qualifying Android run. Require it to independently discover and revalidate the exact-title qualifier run, all four retained-runtime members and records, then verify the deterministic tar, closed staging manifest, exact source-to-staged byte equality, and recorded Actions artifact ID/server digest.
...
13. Dispatch the publisher with the exact source commit, version, annotated tag-object SHA, staging artifact ID/raw-tar digest, and draft Release ID. Read-only preflight must resolve the owning staging run, revalidate every identity and retained-runtime join, and preserve the closed sixteen-asset projection.
14. After `release-production` approval, bind the deterministic provenance-derived release body and attach only the seven binaries, seven sidecars, `SHA256SUMS`, and unchanged raw staging tar. The four runtime members remain inside that tar and the verification receipt remains private. This job must not create or publish a Release, create or move a tag, change the title or prerelease state, or delete or replace any draft asset.
...
Before publication, `official_termux_native_automated_v1` must pass. Its closed envelope is release-qualification evidence, not automated authority to tag or publish.

Automated release qualification proves the exact artifacts under the digest-pinned official Termux userland on native ARM64, including deterministic Android-provider simulation and isolated deployment recovery. It does not certify physical-device, OEM, battery-aging, thermal-soak, radio, Doze, or Android-framework behavior. The retained archive is replayed offline, and the records state `physicalDeviceObserved:false`, `androidFrameworkObserved:false`, `sustainedPhysicalSoak:false`, `physicalCertification:"not_run"`, and `rebuildReproducibilityClaim:false`; they do not claim future rebuild reproducibility.

Optional physical qualification and historical inheritance remain separately named evidence tiers. Neither can be submitted as, merged into, or used to broaden the automated class.
