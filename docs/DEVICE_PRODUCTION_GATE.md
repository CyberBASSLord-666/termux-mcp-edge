...
13. the exact fifteen-tool discovery allowlist, including grant-gated `create_directory`, bounded content-private `copy_file`, content-free bounded `find_paths`, bounded content-private `hash_file`, canonical bounded `read_binary_file`, canonical bounded `read_binary_range`, and bounded `path_metadata` and `search_text`;
