...
- Disabled/enabled write-gate discovery and runtime-status tests with independent shared-key posture.
- Exact-grant write preview/non-consumption, create/replace/content/disposition binding, replay/concurrency, exact 1 MiB plus one, actual-ID response preflight, fixed mode, descriptor-race, cancellation/failure cleanup, exact rollback, and foreign-object preservation tests.
