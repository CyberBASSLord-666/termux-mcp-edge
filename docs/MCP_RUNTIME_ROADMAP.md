...
`main` exposes the health-check runtime by default. The optional `mcp-runtime` feature exposes stable MCP 2025-11-25 Streamable HTTP handling at `/mcp`, exact transport security, bounded sessions, and sixteen baseline tools: deterministic runtime/platform/Android/project-service metadata, preview-first and independently grant-gated single-directory creation, dry-run-first bounded binary file copy, content-free literal basename discovery, streaming bounded SHA-256 file hashing, safe-rooted listing, descriptor-relative single-object metadata, bounded canonical base64 whole-file and range reads, bounded whole-file and code-point-safe ranged UTF-8 reads, bounded literal text search, and preview-first file writing whose live mutation has its own default-disabled, exact-content request-grant gate.
...
Status: exposed behind the staged runtime gate. The surface includes preview-first one-directory creation, dry-run-first one-file binary copy, content-free literal basename discovery, bounded streaming SHA-256 file hashing, deterministic response-bounded listing, bounded single-object metadata, bounded canonical base64 binary reads, bounded UTF-8 reads, bounded literal text-location search, preview-first file writes, explicit crash-durable mutations, and non-sensitive audit counters. Directory mutation is separately default-disabled and requires a 60-second, principal/session/root/target-bound, single-use header grant issued offline by the exact binary. File-write mutation has a different default-false `MCP__FILE__WRITE_MUTATION_ENABLED` gate and a separate 60-second single-use grant bound to principal, session, root, normalized target, exact UTF-8 content, `create` or `replace` disposition, and the exact old file identity for replacement. The offline issuer reads at most 1 MiB from a private stable no-follow content file; MCP never exposes an issuer. The runtime preflights the complete 16 KiB content-free response, safe-root/target state, and per-parent recovery capacity before consumption. It stages a mode-`0600` randomized entry in a fixed mode-`0700` quarantine, uses `NOREPLACE` for creation with no retained artifact, or one irreversible `EXCHANGE` for replacement that preserves the displaced prior inode/content as recovery material. Success reports `recoveryArtifactRetained`; every post-authorization failure retains grant consumption. The quarantine is inaccessible through all MCP filesystem tools and is capped at 32 regular artifacts and 32 MiB per target parent. Live operations retain exact safe-root, source, and destination-parent descriptors through classification, bounded reads, staging, publication, preservation, and durability sync. Path discovery examines at most 8,192 descriptor-relative no-follow entries through depth 5, returns at most 512 ordered file/directory matches, and preflights its complete 262,144-byte response before argument parsing or filesystem access. File copy is capped at 1 MiB, fixes destinations to mode `0600`, returns no content, and preflights its complete 16 KiB response before mutation. Hashing is capped at 16 MiB, retains one exact no-follow descriptor, returns only SHA-256 and bytes hashed, and preflights its complete 16 KiB response before reading. Binary read is capped at 1 MiB raw/1,398,104 bytes encoded, retains one exact no-follow nonblocking descriptor, returns canonical padded base64 without path or host metadata, and preflights its complete 1,507,328-byte response before file access. The response-bound work landed through #206, descriptor-relative race hardening through #200, bounded search through #240, bounded path metadata through #242, bounded directory creation through #244, bounded file copy through #247, directory request authorization through #248, bounded hashing through #261, bounded binary reads through #262/#264, and bounded path discovery through #266.
...
- Deterministic release-validator and Termux device-smoke coverage for disabled/enabled write gates, issuer binding, authorized create/replace, replay denial, reserved-namespace isolation, bounded recovery-artifact retention, redaction, and preserved deployment rollback behavior.
