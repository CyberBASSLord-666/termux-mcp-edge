...
The normal lifetime is 60 seconds. Validation allows at most five seconds of future clock skew and rejects lifetimes above 120 seconds. One process retains at most 4,096 unexpired consumed grant identifiers. Equivalent independently constructed write authorities with the same key identifier, HMAC key, and static principal share that replay set, last-observed clock, and capacity through a bounded process-global registry. Different families, keys, and principals remain isolated. Expired replay entries are pruned before a new valid grant is recorded; a full, poisoned, unavailable, or namespace-exhausted state fails closed. This guarantee does not cross an operating-system process boundary: run one grant consumer for a capability-key/principal domain or add an external atomic one-use coordinator before scaling consumers horizontally.

The runtime first acquires the single shared non-queueing filesystem-mutation worker permit; exhaustion returns private HTTP 503 / JSON-RPC `-32007` before preparation, grant consumption, or mutation. Inside the permit-owned blocking worker it validates safe-root and parent descriptors, target disposition and identity, and content size while constructing the exact grant target binding. After descriptor preparation it acquires one poison-fail-closed process-wide publication lock shared across all directory, file-copy, file-trash, and file-write tool and router instances, preflights quarantine capacity, and revalidates the exact create-or-replace posture. One atomic guard then resolves request cancellation against worker commit ownership. A cancellation winner, including one that arrived while waiting for the process lock, consumes no grant and changes no filesystem state. A stale prepared loser likewise fails before grant consumption and can use the same grant after fresh matching preparation. A worker winner validates and atomically consumes the matching grant immediately before publication work and retains the process lock through verification and durability, continuing independently of later timeout or disconnect; consumption remains authoritative if staging, synchronization, publication, verification, response delivery, or any later step fails.
...
2. acquires the process-wide publication lock, rejects a full, malformed, contended, or unavailable recovery quarantine, and revalidates an absent create target or the exact held and named replace identity;
3. resolves cancellation against worker ownership, then consumes the exact request grant immediately before the first publication mutation;
4. opens or creates the fixed private quarantine, takes its nonblocking advisory lock, and rechecks its contents and capacity;
5. creates one randomized `.termux-mcp-write-artifact-*` regular staging entry at mode `0600`, writes the bounded bytes, synchronizes it, and verifies its held and named identity, type, size, and mode;
6. for `create`, publishes that entry with atomic `NOREPLACE`, leaving no retained recovery artifact;
7. for `replace`, revalidates the bound target as defense in depth and performs one irreversible `EXCHANGE`; the authorized new inode becomes the target and the displaced prior inode/content remains under the randomized quarantine name;
...
- process-lock poison and stale lock-held posture both fail before grant consumption; cancellation before worker commit, including while waiting for the process lock, consumes no grant and changes nothing; cancellation after worker ownership never triggers destructive artifact cleanup or grant replay, and tests account for the possible authorized-new-target plus retained-old-artifact state; and
