...
The runtime first acquires the single shared non-queueing filesystem-mutation worker permit; exhaustion returns private HTTP 503 / JSON-RPC `-32007` before preparation, grant consumption, or mutation. Inside the permit-owned blocking worker it validates safe-root and parent descriptors, target disposition and identity, and content size. It then acquires the process-global filesystem-publication lock shared by every router and live create/write transaction, revalidates the prepared target posture and identity under that lock, and only then resolves request cancellation against worker commit ownership. A stale prepared target or cancellation winner consumes no grant and changes no filesystem state. A worker winner preflights quarantine capacity, atomically consumes the grant immediately before publication work, and holds the global lock through publication and verification. It continues independently of later timeout or disconnect; consumption remains authoritative if staging, synchronization, publication, verification, response delivery, or any later step fails.
...
2. acquires the process-global publication lock, revalidates the exact create-or-replace posture and held replacement identity, resolves cancellation against worker ownership, rejects a full, malformed, contended, or unavailable recovery quarantine, then consumes the exact request grant immediately before the first publication mutation;
