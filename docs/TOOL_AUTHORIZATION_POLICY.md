...
- Require an operation-scoped authorization grant when the mutation's impact exceeds the bearer principal's baseline preview posture; directory and write grants use [`CREATE_DIRECTORY_CAPABILITY_GRANTS.md`](CREATE_DIRECTORY_CAPABILITY_GRANTS.md) and [`WRITE_FILE_CAPABILITY_GRANTS.md`](WRITE_FILE_CAPABILITY_GRANTS.md).
...
2. Reflect the gate posture in discovery before returning tool schemas. Disabled `create_directory` and `write_file` postures constrain `dry_run` to `true`; enabled postures advertise that an exact request grant is still required.
...
The static token authenticates the complete MCP route before discovery or invocation. Live directory and file-write grants are additive, header-only, exact-operation-bound, and fail closed; a session ID, shared signing key, another capability's grant, or `dry_run:false` never substitutes for the matching grant.
