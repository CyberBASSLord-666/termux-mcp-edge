...
`android_rish_status` is Class 1 only under the narrower development contract in [`SHIZUKU_RISH_CONTROL_PLANE.md`](SHIZUKU_RISH_CONTROL_PLANE.md): static authentication, isolated compile/runtime gates, one pinned private DEX, one fixed loader invocation, exact `2000\n` bytes in exactly one local capture, stable non-reflective failures, and no parameters. The observation is not remote-exit, stream-separation, Binder-identity, or lifecycle proof. Any package, settings, intent, process, input, capture, storage, network, radio, power, user, diagnostic, or mutation operation is a separate high-impact capability family and remains blocked.
