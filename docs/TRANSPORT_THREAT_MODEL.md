...
- the seven-tool baseline allowlist, with an eighth read-only battery tool only when its independent compile/runtime gates are both enabled, as documented in README and the authorization policy;
