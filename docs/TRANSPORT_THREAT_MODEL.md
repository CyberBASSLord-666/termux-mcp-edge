...
- an independently default-disabled `MCP__FILE__TRASH_FILE_MUTATION_ENABLED` gate plus one 60-second, single-use HMAC grant bound to the principal, session, pinned root, normalized target, exact single-link identity/content, and recovery-retaining posture;
