...
- The shared configured HMAC key plus the independently capability-bound directory, file-copy, file-trash, file-write, and volume grants, consumed-JTI state, and target/content/disposition/identity/recovery-posture binding integrity.
...
- create/copy/trash/write grant issuance, runtime target preparation, and consumption bound to the same pinned root identity, including rejection when issuance sees a later pathname replacement;
...
- an independently default-disabled `MCP__FILE__TRASH_FILE_MUTATION_ENABLED` gate plus one 60-second, single-use HMAC grant bound to the static principal, canonical session, exact root/path, single-link target device/inode/size/high-resolution ctime, SHA-256 content, and fixed recovery-retained posture;
...
- one-file reversible trashing with a separate per-parent mode-`0700` `.termux-mcp-trash-quarantine`, an unpredictable absent recovery name, atomic `NOREPLACE` movement of the exact authorized inode, retained identity/content verification, and no MCP purge or restore;
...
- both quarantine namespaces isolated from all MCP filesystem operations, each capped at 32 regular artifacts and 32 MiB per parent, with nonblocking advisory locks for cooperating writers;
- cancellation-independent completion after worker ownership and grant consumption, fixed 1 MiB mutation content ceilings, and bounded private 16 KiB complete responses.

The write exchange and trash rename are their respective commit points. A later identity or durability failure can leave the authorized new write inode public with the displaced object quarantined, or the exact trashed inode retained with its public name absent; either request is denied and its grant stays consumed. The preservation policy avoids destructive cleanup against an uncertain name, but it is not atomic rollback against a malicious process under the same Unix UID. Such a peer can ignore advisory locks, mutate names, or exhaust per-parent capacity, so the limits bound cooperating runtime retention rather than global disk use. Production deployment therefore requires exclusive operational ownership of mutation safe roots by this service, with no independent writers active while live create, copy, trash, or write gates are enabled.
...
- grants are issued only by the local exact binary after fallible construction independently lifetime-pins the configured safe root and validates the capability-specific target. The signed binding uses that pin's device/inode identity, and runtime consumption compares it with the identity retained by the running process; a grant issued after pathname replacement cannot authorize the earlier pin. Copy and trash issuers open and hash the exact bounded file themselves; the write issuer reads exact UTF-8 bytes from an absolute stable no-follow private regular content file and validates the requested `create`/`replace` target state;
- every request-grant family uses an opaque fixed-shape signed payload containing only a random JTI, signed family byte, keyed domain-separated operation binding, and issued/expiry timestamps. Principal, canonical session, root/path, identity/content, disposition, recovery posture, and other operation-specific inputs are bound but not serialized;
...
- after descriptor preparation, one poison-fail-closed process-wide publication lock serializes every `create_directory`, `copy_file`, `trash_file`, and `write_file` instance and transport state through posture revalidation, grant consumption, mutation, verification, and durability. A stale prepared loser fails before grant consumption. While holding that lock, a reusable atomic commit guard races request cancellation against worker ownership immediately before grant consumption. A cancellation winner, including one that arrived while waiting for the process lock, consumes no grant and mutates nothing. A worker winner consumes the exact directory, copy, trash, or write grant immediately before its first namespace mutation and owns completion; consumption then survives staging, retention, sync, exchange, recovery, publication, response, timeout, and disconnect outcomes;
...
- the registry is process-local. Separate operating-system processes configured with the same key material have independent replay and clock state. Run one grant-consuming process per capability-key/principal domain, or introduce an external atomic one-use coordinator before a multi-process deployment.
...
- empty environment, null stdin, immutable 5-second/16-KiB/4-KiB supervisor maxima, two non-queueing permits, process-group cleanup, zero-exit/UTF-8 success, and non-sensitive audit reasons are enforced; oversize configuration fails before spawn and capacity grows fallibly only from bytes actually read;
...
- the narrow `create_directory`, `copy_file`, `trash_file`, `write_file`, and exact-stream volume request-grant modules are live only for their distinct bound mutations; the separate general capability-token policy module remains inert;
- public directory-creation, copy, trash, file-write, and Android-volume library APIs expose preview only. Their live preparation and execution are crate-private, preventing an embedding from routing around runtime gates, session binding, response preflight, cancellation ownership, grant consumption, and terminal audit ownership. Public copy/trash target inspection is non-mutating and exists only for exact-binary offline issuance;
