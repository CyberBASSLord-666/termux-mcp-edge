...
- every embedded router also shares one process-global filesystem-publication lock. Under it, create/write revalidate the prepared target posture and replacement identity before the reusable atomic guard races request cancellation against worker ownership. A stale target or cancellation winner consumes no grant and mutates nothing. A worker winner consumes the exact grant immediately before its first mutation and holds the lock through verified publication; consumption then survives staging, sync, exchange, recovery, response, timeout, and disconnect outcomes;
