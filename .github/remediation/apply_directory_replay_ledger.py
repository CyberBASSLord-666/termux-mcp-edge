...
Path("src/directory_replay.rs").write_text(r'''//! Crash-durable, bounded, single-use replay consumption for directory grants.
//!
...
# Add test support before tests module.
replace_once(
    grant,
    '''#[cfg(test)]
mod tests {
...
# Existing DirectoryGrantConfig test literals need replay defaults.
grant_text = grant.read_text()
needle = '''            max_lifetime_seconds: 120,
            clock_skew_seconds: 5,
        }
'''
replacement = '''            max_lifetime_seconds: 120,
            clock_skew_seconds: 5,
            replay_enabled: false,
            replay_keyring_path: None,
            replay_ledger_path: None,
...
'''
count = grant_text.count(needle)
if count != 1:
    raise SystemExit(f"src/directory_grant.rs: expected one legacy config literal, found {count}")
grant.write_text(grant_text.replace(needle, replacement, 1))
...
# Add replay defaults to all DirectoryGrantConfig literals in config.rs.
...
'''
count = config_text.count(needle)
if count != 1:
    raise SystemExit(f"src/config.rs: expected one app_config replay default block, found {count}")
config_text = config_text.replace(needle, replacement, 1)
needle2 = '''            max_lifetime_seconds: 120,
            clock_skew_seconds: 5,
        };
'''
replacement2 = '''            max_lifetime_seconds: 120,
            clock_skew_seconds: 5,
            replay_enabled: false,
            replay_keyring_path: None,
            replay_ledger_path: None,
...
'''
count = config_text.count(needle2)
if count != 2:
    raise SystemExit(f"src/config.rs: expected two test config blocks, found {count}")
config_text = config_text.replace(needle2, replacement2)
