...
- Added the opt-in `android_battery_status` MCP tool behind independent `android-battery-status` compile and `MCP__ANDROID__BATTERY_STATUS_ENABLED` runtime gates, with a fixed absolute Termux:API executable, zero caller arguments, cleared environment, null stdin, a five-second end-to-end deadline, independent output limits, strict normalized field allowlisting, stable redacted failures, hidden disabled discovery, and aggregate audit coverage.
- Hardened the battery provider with a cancellation-safe single supervisor, isolated process-group termination, immediate overflow handling, deadline-bounded pipe completion, synchronous direct-child reaping, stable terminal-event precedence, and repeated endless-output, pipe-holding descendant, caller-cancellation, survivor, and task-accumulation regressions.
