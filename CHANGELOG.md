...
- Hardened the battery provider with a cancellation-safe single supervisor, isolated process-group termination, immediate overflow handling, deadline-bounded pipe completion, authoritative synchronous direct-child reaping, stable terminal-event precedence, and repeated endless-output, pipe-holding descendant, caller-cancellation, forced cleanup-reserve exhaustion, survivor, and task-accumulation regressions. Cleanup that misses its reserve now overrides every primary result with a stable wait failure only after confirmed reaping.
