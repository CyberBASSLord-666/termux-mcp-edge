...
- Added the exact-binary `--issue-create-directory-grant` offline issuer, header-only transport, disabled/enabled discovery metadata, stable private authorization denials, post-consumption failure semantics, CLI/config/deployment/adversarial regressions, and exact-candidate release/device/native-emulated validation for missing, wrong-context, mismatched, dry-run, success, existing-target, and replay paths.
