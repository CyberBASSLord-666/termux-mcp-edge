...
- Extended closed MCP schemas, 16 KiB pre-mutation full-response enforcement, stable copy-specific errors and low-cardinality private audit reasons, exact-limit/binary/empty/cross-root/same/existing/missing/type/symlink/outside/race/cleanup regressions, release-validator fixtures, device smoke, and native/emulated Android gates for the eleven-tool baseline.
...
- Added the opt-in `run_command_profile` MCP tool behind independent `command-execution` compile and `MCP__COMMAND__ENABLED` runtime gates. It exposes only three fixed read-only diagnostics of the exact current server executable, with a canonical safe-root cwd, cleared environment, null stdin, five-second profile deadlines, independent stdout/stderr ceilings, a two-permit non-queueing concurrency limit, UTF-8 zero-exit responses, stable redacted failures, hidden disabled discovery, and aggregate audit coverage.
