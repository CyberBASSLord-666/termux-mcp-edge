...
if output="$(bash "$SCRIPT" rollback --dry-run 2>&1)"; then fail_test "copy_file gate without static authentication unexpectedly succeeded"; fi
grep -Fq 'copy_file mutation requires static-token authentication' <<<"$output" || fail_test "copy_file static-auth dependency failure was not explicit"
...
if output="$(bash "$SCRIPT" rollback --dry-run 2>&1)"; then fail_test "copy_file gate without capability keys unexpectedly succeeded"; fi
grep -Fq 'copy_file mutation requires capability key configuration' <<<"$output" || fail_test "copy_file key dependency failure was not explicit"
sed -i 's/^MCP__FILE__COPY_FILE_MUTATION_ENABLED=true$/MCP__FILE__COPY_FILE_MUTATION_ENABLED=false/' "$TERMUX_MCP_CONFIG_ROOT/runtime.env"
bash "$SCRIPT" rollback --dry-run >/dev/null
