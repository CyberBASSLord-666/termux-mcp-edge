...
async fn write_file_allows_exact_default_payload_limit_with_explicit_mutation() {
    ...
    let result = tools.write_file(target.to_string_lossy().to_string(), content, Some(false)).await.unwrap();
...
async fn write_file_allows_exact_default_payload_limit_with_dry_run() {
    ...
    let result = tools.write_file(target.to_string_lossy().to_string(), content, None).await.unwrap();
